...
for /f "delims=" %%F in ('"%PY%" -c "import glob,os;cwd=os.getcwd();print(next(p for p in glob.glob(os.path.join(cwd,'quant_trading_system','dashboard','*.py')) if os.path.basename(p)=='home.py'))"') do set ENTRY=%%F
